...
const Navbar = () => {
  ...
  return <nav className={`site-navbar fixed top-0 left-0 right-0 z-50 border-b border-border bg-background/80 backdrop-blur-xl ${scrolled ? "site-navbar--scrolled" : ""}`}>
      {
    /* Scroll progress bar */
  }
      <motion.div
  ...
  />
      ...
      <div className="container mx-auto flex items-center justify-between px-6 py-3.5">
        <div className="flex items-center gap-4">
          <a href="#home" className="navbar-brand font-mono text-lg font-semibold text-primary flex items-center gap-2" aria-label="Go to Home">
            <span className="navbar-brand__signal" aria-hidden="true" />
            <span>{"< ARA />"}</span>
          </a>
        </div>
    ...
    className={`navbar-link text-sm font-medium transition-colors hover:text-primary hover-underline pb-1 ${activeSection === l.href.slice(1) ? "navbar-link--active text-primary" : "text-muted-foreground"}`}
              ...
              >
                {navLinks.map((l, i) => <motion.li key={l.href} variants={itemVariants}>
                    <a
                      ...
                      className={`navbar-mobile-link block rounded-lg py-2.5 text-sm font-medium transition-all duration-200 hover:text-primary hover:pl-2 ${activeSection === l.href.slice(1) ? "navbar-mobile-link--active text-primary" : "text-muted-foreground"}`}
                      aria-current={activeSection === l.href.slice(1) ? "page" : undefined}
                    >
